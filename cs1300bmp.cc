  ...
  //
{
  ...
  indexr = rarray;
  numbyte = 0;
  int unroll_width = width - 3;
  int absHeight = abs (height);

  for ( int  j = 0; j < absHeight; j++ ) 
    {
      for ( i = 0; i < unroll_width; i = i + 4)
	{
	  file_in.read ( &c, 4 );
	  ...
	  if ( error ) 
	    {
	      ...
	      cout << "  Failed reading R for pixel (" << i-3 << "," << j-3 << ").\n";
	      return error;
	    }
	  ...
	  if ( error )
	    {
	      ...
	      cout << "  Failed while reading padding character " << i-3 << "\n";
	      cout << "  of total " << padding << " characters\n";
	      cout << "  at the end of line " << j-3 << "\n";
	      cout << "\n";
  ...
  //
{
  ...
  for ( int j = 0; j < absHeight; j++ )
      ...
      for (; i < width; i++ )
	{
      ...
      int i;
      int unroll_padding = padding-7;

      for (i = 0; i < unroll_padding; i = i + 8 )
	{
	  file_out << 0;
	  file_out << 0;
	  file_out << 0;
...
inline bool bmp_header1_read ( ifstream &file_in, unsigned short int *filetype, 
  ...
  //
{
  ...
  if ( bmp_byte_swap )
    {
      i1 = ( char ) ( *filetype >> 8); // same as *filetype / 256
      i2 = ( char ) ( *filetype & 255); // same as *filetype % 256
      *filetype = (i2 << 8) + i1; // same as i2 * 256 +i1
    }
  //
  //  Read FILESIZE.
  //
...
inline void bmp_header1_write ( ofstream &file_out, unsigned short int filetype,
			 unsigned long int filesize, unsigned short int reserved1, 
			 unsigned short int reserved2, unsigned long int bitmapoffset )
...
inline static bool bmp_header2_read ( ifstream &file_in, unsigned long int *size,
			unsigned long int *width, long int *height, 
			unsigned short int *planes, unsigned short int *bitsperpixel,
			unsigned long int *compression, unsigned long int *sizeofbitmap,
			unsigned long int *horzresolution, unsigned long int *vertresolution,
...
inline static void bmp_header2_write ( ofstream &file_out, unsigned long int size,
			 unsigned long int width, long int height, 
			 unsigned short int planes, unsigned short int bitsperpixel,
			 unsigned long int compression, unsigned long int sizeofbitmap,
			 unsigned long int horzresolution, unsigned long int vertresolution,
...
inline static bool bmp_24_write ( char *file_out_name, unsigned long int width, long int height, 
		    unsigned char *rarray, unsigned char *garray, unsigned char *barray )
  ...
  //
{
  ...
  int absHeight =  abs ( height );
  filesize = 54 + ( ( (width << 1) + width ) + padding ) * absHeight;
  bitmapoffset = 54;
...
inline static bool long_int_read ( long int *long_int_val, ifstream &file_in )
...
inline static void long_int_write ( long int long_int_val, ofstream &file_out )
...
inline static bool u_long_int_read ( unsigned long int *u_long_int_val, 
		       ifstream &file_in )
...
inline static void u_long_int_write ( unsigned long int u_long_int_val, 
			ofstream &file_out )
...
inline static bool u_short_int_read ( unsigned short int *u_short_int_val, 
			ifstream &file_in )
...
inline static void u_short_int_write ( unsigned short int u_short_int_val, 
			 ofstream &file_out )
...
cs1300bmp_readfile(char *filename, struct cs1300bmp *image)
{
  ...
  if ( error ) {
	...
	image -> color[COLOR_RED  ][row + 1][col] = rarray[(row + 1) * width + col];
	image -> color[COLOR_GREEN][row + 1][col] = garray[(row + 1) * width + col];
	image -> color[COLOR_BLUE ][row + 1][col] = barray[(row + 1) * width + col];
...
cs1300bmp_writefile(char *filename, struct cs1300bmp *image)
{
  ...
  int width  = image -> width;
    ...
    for (int col = 0; col < width; col ++ ) {
      for (row = 0; row < unroll_height; row = row + 2) {
	...
	rarray[(row + 1) * width + col] = image -> color[COLOR_RED  ][row + 1][col];
	garray[(row + 1) * width + col] = image -> color[COLOR_GREEN][row + 1][col];
	barray[(row + 1) * width + col] = image -> color[COLOR_BLUE ][row + 1][col];
      }
      for (; row < height; row ++ ) {
